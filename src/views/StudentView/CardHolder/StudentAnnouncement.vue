    ...
    <v-data-iterator :items="items" :items-per-page.sync="itemsPerPage" :page.sync="page" :search="search"
      ...
      <template v-slot:header>
        <v-toolbar dark color="#80a211" class="mb-1">
          ...
          <template v-if="$vuetify.breakpoint.mdAndUp">
            <v-spacer></v-spacer>
            <v-select v-model="sortBy" flat color="#116530" solo-inverted hide-details :items="keys"
              prepend-inner-icon="mdi-magnify" label="Sort by"></v-select>
            <v-spacer></v-spacer>
            <v-btn-toggle v-model="sortDesc" mandatory>
              <v-btn medium depressed color="#1B5E20" :value="false">
                <v-icon>mdi-arrow-up</v-icon>
              </v-btn>
              <v-btn medium depressed color="#1B5E20" :value="true">
      ...
      <template v-slot:footer>
        <v-row class="mt-2" align="center" justify="center">
          ...
          <v-btn fab dark small color="#116530" class="mr-1" @click="formerPage">
            <v-icon>mdi-chevron-left</v-icon>
          </v-btn>
          <v-btn fab dark small color="#116530" class="ml-1" @click="nextPage">
